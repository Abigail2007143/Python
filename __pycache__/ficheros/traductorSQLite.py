...
conexion.execute("insert into traductor(espanol,ingles) values (?,?)",('lapíz','pen'))
conexion.execute("insert into traductor(espanol,ingles) values (?,?)",('oro','gold'))
conexion.commit()
conexion.close()

while True:
    print("traducir ESPAÑOL A INGLES")
    palabra= input("ingrese una palabra a traducir :")
    conexion=sqlite3.connect("bdTraductor.db")
    sentencia= f"select ingles from Tradcutor where espanol='{palabra}'"
